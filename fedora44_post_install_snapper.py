...
class PreFlightChecks(InstallStep):
    ...
    label = "1/20 — Verificações iniciais"
...
class RepositorySetup(InstallStep):
    ...
    label = "2/20 — Repositórios (RPM Fusion + Flathub)"
    ...
    def _setup_rpmfusion(self) -> None:
        ...
        fedora_ver = run(["rpm", "-E", "%fedora"], capture=True).stdout.strip()
        for variant in ("free", "nonfree"):
            url = (
                f"https://mirrors.rpmfusion.org/{variant}/fedora/"
                f"rpmfusion-{variant}-release-{fedora_ver}.noarch.rpm"
            )
            try:
...
class SystemUpdate(InstallStep):
    ...
    label = "3/20 — Atualização do sistema"

    def execute(self) -> None:
        self.info("Executando dnf upgrade --refresh...")
...
class KernelTuning(InstallStep):
    ...
    label = "5/20 — Tuning de kernel e I/O (sysctl, THP, NVMe, btrfs)"
    ...
    def _optimize_btrfs_mount(self) -> None:
        self.info("Verificando otimizações btrfs no fstab...")
        fstab = Path("/etc/fstab")
        if not fstab.exists():
            self.warn("/etc/fstab não encontrado")
            return
...
class ZramSetup(InstallStep):
    ...
    label = "6/20 — ZRAM (swap comprimido em RAM)"
...
class CpuPowerManagement(InstallStep):
    ...
    label = "7/20 — CPU: AMD pstate + tuned + auto-cpufreq"
...
class JavaBackendInstaller(InstallStep):
    ...
    label = "9/20 — Java Backend (SDKMan, Java 21, Maven, Gradle, Quarkus)"
...
class GnomeThemeSetup(InstallStep):
    ...
    label = "14/20 — Tema Fluent Blue Dark + ícones"
...
class WallpaperSetup(InstallStep):
    ...
    label = "15/20 — Wallpaper Firewatch Dinâmico"
...
class SnapperSetup(InstallStep):
    ...
    label = "17/20 — Snapper (snapshots automáticos via btrfs)"

    SNAPPER_ROOT_CONFIG = "root"

    def execute(self) -> None:
        self._install_snapper()
        self._create_snapper_config()
        self._enable_snapper_timers()
        self._configure_auto_cleanup()
        self.ok("Snapper configurado — snapshots automáticos ativados")

    def _install_snapper(self) -> None:
        self.info("Instalando snapper e dependências...")
        try:
            dnf("snapper", "dnf-plugin-snapper")
            self.ok("snapper instalado")
        except CommandError as exc:
...
class DotfilesSetup(InstallStep):
    ...
    label = "18/20 — Dotfiles e ZSH config"
...
class DisableUnnecessaryServices(InstallStep):
    ...
    label = "19/20 — Desativando serviços lentos"
